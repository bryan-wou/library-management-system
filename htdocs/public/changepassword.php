...
if (!empty($_POST['do']) && $_POST['do'] == 'changepassword') {
  ...
  $user = DB::queryFirstRow('SELECT * FROM Patron p WHERE p.patronID = %i', $_SESSION['PatronID']);
  if (password_verify(($_POST['cpw']??''),($user['password']??''))) {
    // log in successful
    if ($_POST['npw'] == $_POST['cnpw']) {
      DB::update('Patron',array('password'=>password_hash($_POST['npw'],PASSWORD_DEFAULT)),'patronID=%i',$_SESSION['PatronID']);
