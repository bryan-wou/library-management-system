...
require_once __DIR__.'/../plugins/pomo/vendor/autoload.php';
use POMO\MO;
$translations = new MO();

if (isset($_SESSION['Locale'])) {
  $_SESSION['Locale'] = preg_replace("/[^A-Za-z0-9_]/", '', $_SESSION['Locale']);
  if (file_exists(__DIR__.'/../user/locales/'.$_SESSION['Locale'].'.mo')) {
    $translations->import_from_file(__DIR__.'/../user/locales/'.$_SESSION['Locale'].'.mo');
    define('LMS_CURRENT_LOCALE', (LMS_LOCALES_NAME[$_SESSION['Locale']]??$_SESSION['Locale']));
  }
}

function lms_get_available_locales(){
  $locales = [];
  $newlocales = array_diff(scandir(__DIR__.'/../user/locales/'), array('.', '..'));
  foreach ($newlocales as $locale) { $locales[] = explode('.',$locale)[0]; }
  return $locales;
}

function lms_set_locale($locale){
  if ($locale=='default') { unset($_SESSION['Locale']); } 
  elseif (file_exists(__DIR__.'/../user/locales/'.$locale.'.mo')) { $_SESSION['Locale'] = $locale; }
  return true;
}

function lms_get_translation($text){
	global $translations;
	return $translations->translate($text);
}

function lms_echo_translation($text){
  echo lms_get_translation($text);
}

function lms_escape_html($text){
  return htmlspecialchars($text);
}

function __($text){
	return lms_get_translation($text);
}

function _e($text){
	return lms_echo_translation($text);
}

function esc_html__($text){
	return lms_get_translation(lms_escape_html($text));
}

function esc_html_e($text){
	return lms_echo_translation(lms_escape_html($text));
}

function esc_html($text){
  return lms_escape_html($text);
}
